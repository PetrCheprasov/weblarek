...
// Проверка методов каталога и API
...
// Загрузка данных каталога с обработкой ошибок
comApi.get('/product/')
  .then(res => {
...
// Проверка метода API post с обработкой ошибок
comApi.post('/order/', data)
  .then(resp => {
    console.log('Успешный ответ от сервера:', resp);
  })
  .catch(error => {
